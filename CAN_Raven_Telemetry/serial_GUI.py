...
bms_peak_temps = {0,0,0,0,0,0,0,0};
def reset_bms_temp(pack_num, temp_num):
        row_count = 2 if pack_num is 3 else 3;
        var["pack"][pack_num][row_count + temp_num][2].set("N/A");
        var["pack"][pack_num][row_count + temp_num][3].set("N/A");
        bms_peak_temps[2*pack_num] = 0;
        bms_peak_temps[2*pack_num + 1] = 0;
...
for i in range(4):
        ...
        for k in range(2):
                ...
                Button(subsubframe, text = "RESET", width = 10, command = lambda:reset_bms_temp(i, k)).grid(row = k + 1, column = 5);
